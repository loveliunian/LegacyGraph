export default {
  common: {
    confirm: '确认',
    cancel: '取消',
    save: '保存',
    delete: '删除',
    edit: '编辑',
    add: '添加',
    search: '搜索',
    reset: '重置',
    export: '导出',
    import: '导入',
    refresh: '刷新',
    loading: '加载中...',
    success: '成功',
    error: '失败',
    warning: '警告',
    info: '信息',
    noData: '暂无数据',
    operation: '操作',
    status: '状态',
    createTime: '创建时间',
    updateTime: '更新时间',
    actions: '操作',
    view: '查看',
    detail: '详情',
    back: '返回',
    backToHome: '返回首页',
    submit: '提交',
    close: '关闭',
    enable: '启用',
    disable: '禁用',
    enabled: '已启用',
    disabled: '已禁用',
    all: '全部',
    selected: '已选择',
    items: '项',
    selectAll: '全选',
    cancelAll: '取消全选',
    deleteConfirm: '确认删除？',
    deleteSuccess: '删除成功',
    saveSuccess: '保存成功',
    updateSuccess: '更新成功',
    createSuccess: '创建成功',
    uploadSuccess: '上传成功',
    downloadSuccess: '下载成功',
    operationSuccess: '操作成功',
    operationFailed: '操作失败',
    networkError: '网络错误，请稍后重试',
    serverError: '服务器错误',
    unauthorized: '未授权，请重新登录',
    forbidden: '没有权限访问',
    notFound: '资源不存在',
    required: '必填项',
    invalidFormat: '格式不正确',
    tooShort: '输入内容太短',
    tooLong: '输入内容太长',
    minLength: '最少 {min} 个字符',
    maxLength: '最多 {max} 个字符',
    yes: '是',
    no: '否'
  },
  auth: {
    login: '登录',
    logout: '退出登录',
    username: '用户名',
    password: '密码',
    rememberMe: '记住我',
    forgotPassword: '忘记密码',
    loginSuccess: '登录成功',
    loginFailed: '登录失败',
    pleaseLogin: '请先登录',
    invalidCredentials: '用户名或密码错误',
    accountLocked: '账号已锁定',
    accountDisabled: '账号已禁用',
    sessionExpired: '会话已过期，请重新登录',
    logoutConfirm: '确认退出登录？',
    logoutSuccess: '已退出登录',
    noPermission: '没有权限访问该页面',
    tokenInvalid: 'Token 无效或已过期'
  },
  menu: {
    dashboard: '仪表盘',
    projects: '项目管理',
    projectList: '项目列表',
    projectDetail: '项目详情',
    sources: '数据源',
    codeRepos: '代码仓库',
    databases: '数据库',
    documents: '文档',
    scan: '扫描管理',
    scanVersions: '扫描版本',
    graph: '知识图谱',
    codeGraph: '代码图谱',
    unifiedGraph: '统一图谱',
    businessGraph: '业务图谱',
    featureGraph: '功能图谱',
    dataLineage: '数据血缘',
    runtimeGraph: '运行链路',
    review: '人工审核',
    reviewList: '待审核列表',
    reviewHistory: '审核历史',
    facts: '事实管理',
    factList: '事实列表',
    evidenceSearch: '证据检索',
    test: '测试用例',
    testCaseList: '测试用例列表',
    testCaseEditor: '测试用例编辑',
    testRunList: '测试执行记录',
    testRunDetail: '测试执行详情',
    reports: '报告中心',
    validationReport: '验证报告',
    migration: '迁移辅助',
    migrationRisks: '迁移风险',
    riskDetail: '风险详情',
    audit: '审计日志',
    auditLogs: '操作日志',
    logDetail: '日志详情',
    system: '系统管理',
    users: '用户管理',
    dictionaries: '字典管理',
    settings: '系统设置',
    profile: '个人中心',
    notifications: '通知中心',
    evidenceWorkbench: '证据工作台',
    changeTasks: '变更任务',
    graphQa: '图谱问答',
    agentHub: 'AI 助手',
    understanding: '代码理解',
    vectorSearch: '向量检索',
    prWorkbench: 'PR 工作台',
    graphDiff: '图谱对比',
    graphifyJobs: 'Graphify 作业中心',
    graphifyDiff: 'Graphify 版本差异',
    graphifyQuality: 'Graphify 质量仪表盘',
    crossRepoImpact: '跨仓影响分析',
    knowledgeWorkbench: '知识工作台',
    agentHistory: 'Agent 运行历史',
    evidenceConflict: '证据冲突处理',
    systemOverview: '系统关系总览',
    llmProvider: 'LLM 提供商',
    promptTemplates: '提示词管理',
    plugins: '插件管理',
    requirementAnalysis: '需求分析',
    solutionReview: '方案评审'
  },
  dashboard: {
    title: '仪表盘',
    welcome: '欢迎回来',
    totalProjects: '项目总数',
    totalNodes: '节点总数',
    totalEdges: '关系总数',
    pendingReviews: '待审核项',
    scanProgress: '扫描进度',
    recentActivities: '最近活动',
    quickActions: '快捷操作',
    startScan: '开始扫描',
    createProject: '创建项目',
    viewGraph: '查看图谱',
    viewReports: '查看报告',
    recentScans: '最近扫描',
    topRisks: 'Top 风险项',
    qualityTrend: '质量趋势',
    nodeDistribution: '节点分布'
  },
  project: {
    title: '项目',
    projectName: '项目名称',
    projectDesc: '项目描述',
    projectType: '项目类型',
    projectStatus: '项目状态',
    owner: '负责人',
    createProject: '创建项目',
    editProject: '编辑项目',
    projectInfo: '项目信息',
    projectOverview: '项目概览',
    statistics: '统计信息',
    totalSources: '数据源总数',
    totalScans: '扫描次数',
    lastScanTime: '最后扫描时间',
    avgConfidence: '平均置信度',
    coverageRate: '覆盖率',
    webApp: 'Web应用',
    backend: '后端服务',
    mobile: '移动应用',
    desktop: '桌面应用',
    active: '进行中',
    completed: '已完成',
    archived: '已归档'
  },
  source: {
    title: '数据源',
    repoUrl: '仓库地址',
    repoType: '仓库类型',
    branch: '分支',
    lastSync: '最后同步时间',
    syncStatus: '同步状态',
    syncNow: '立即同步',
    addRepo: '添加仓库',
    editRepo: '编辑仓库',
    deleteRepo: '删除仓库',
    connectionString: '连接字符串',
    dbType: '数据库类型',
    dbName: '数据库名',
    testConnection: '测试连接',
    connectionSuccess: '连接成功',
    connectionFailed: '连接失败',
    documentTitle: '文档标题',
    documentType: '文档类型',
    fileSize: '文件大小',
    uploadDate: '上传日期',
    uploadDocument: '上传文档',
    synced: '已同步',
    syncing: '同步中',
    failed: '同步失败',
    pending: '待同步',
    github: 'GitHub',
    gitlab: 'GitLab',
    gitee: 'Gitee',
    local: '本地目录',
    mysql: 'MySQL',
    postgresql: 'PostgreSQL',
    oracle: 'Oracle',
    sqlserver: 'SQL Server',
    word: 'Word文档',
    pdf: 'PDF文档',
    markdown: 'Markdown',
    excel: 'Excel表格'
  },
  scan: {
    title: '扫描',
    scanVersion: '扫描版本',
    scanTime: '扫描时间',
    scanStatus: '扫描状态',
    scanDuration: '扫描耗时',
    totalFiles: '文件总数',
    codeLines: '代码行数',
    newNodes: '新增节点',
    newEdges: '新增关系',
    startScan: '开始扫描',
    stopScan: '停止扫描',
    scanConfig: '扫描配置',
    scanMode: '扫描模式',
    fullScan: '全量扫描',
    incrementalScan: '增量扫描',
    scanScope: '扫描范围',
    scanProgress: '扫描进度',
    scanning: '扫描中',
    completed: '已完成',
    failed: '失败',
    cancelled: '已取消',
    queued: '排队中',
    scanLog: '扫描日志',
    scanResult: '扫描结果',
    qualityReport: '质量报告'
  },
  graph: {
    title: '知识图谱',
    codeGraph: '代码图谱',
    unifiedGraph: '统一图谱',
    nodeType: '节点类型',
    edgeType: '关系类型',
    confidence: '置信度',
    zoomIn: '放大',
    zoomOut: '缩小',
    fitView: '适应视图',
    centerView: '居中显示',
    fullscreen: '全屏',
    searchNode: '搜索节点',
    filter: '筛选',
    layout: '布局',
    forceLayout: '力导向布局',
    circularLayout: '环形布局',
    gridLayout: '网格布局',
    layeredLayout: '分层布局',
    nodeDetail: '节点详情',
    relatedNodes: '关联节点',
    pathAnalysis: '路径分析',
    impactAnalysis: '影响分析',
    showLegend: '显示图例',
    exportImage: '导出图片',
    exportJson: '导出JSON',
    businessDomain: '业务域',
    businessProcess: '业务流程',
    featureModule: '功能模块',
    feature: '功能点',
    api: 'API接口',
    controller: 'Controller',
    service: 'Service',
    mapper: 'Mapper',
    sqlStatement: 'SQL语句',
    table: '数据库表',
    column: '字段'
  },
  review: {
    title: '审核',
    pendingReview: '待审核',
    reviewHistory: '审核历史',
    reviewStatus: '审核状态',
    reviewer: '审核人',
    reviewTime: '审核时间',
    reviewComment: '审核意见',
    approve: '通过',
    reject: '拒绝',
    batchApprove: '批量通过',
    batchReject: '批量拒绝',
    approved: '已通过',
    rejected: '已拒绝',
    pending: '待审核',
    evidence: '证据',
    confidence: '置信度',
    suggestion: '建议',
    autoReview: '自动审核',
    manualReview: '人工审核',
    conflictDetection: '冲突检测',
    noConflict: '无冲突',
    hasConflict: '存在冲突',
    reviewRule: '审核规则',
    reviewQuality: '审核质量'
  },
  test: {
    title: '测试',
    testCase: '测试用例',
    caseName: '用例名称',
    caseType: '用例类型',
    casePriority: '优先级',
    testSteps: '测试步骤',
    expectedResult: '预期结果',
    actualResult: '实际结果',
    testData: '测试数据',
    assertion: '断言',
    generateCase: '生成用例',
    runTest: '运行测试',
    runAll: '运行全部',
    runSelected: '运行选中',
    testResult: '测试结果',
    testPassed: '通过',
    testFailed: '失败',
    testSkipped: '跳过',
    testError: '错误',
    passRate: '通过率',
    coverage: '覆盖率',
    testReport: '测试报告',
    unitTest: '单元测试',
    integrationTest: '集成测试',
    e2eTest: '端到端测试',
    apiTest: 'API测试',
    performanceTest: '性能测试',
    securityTest: '安全测试',
    highPriority: '高',
    mediumPriority: '中',
    lowPriority: '低',
    p1: 'P1 最高',
    p2: 'P2 高',
    p3: 'P3 中',
    p4: 'P4 低'
  },
  migration: {
    title: '迁移辅助',
    riskAssessment: '风险评估',
    riskLevel: '风险等级',
    riskType: '风险类型',
    riskDescription: '风险描述',
    affectedModules: '影响模块',
    affectedNodes: '影响节点',
    suggestion: '改进建议',
    codeRefactoring: '代码重构',
    dataMigration: '数据迁移',
    compatibility: '兼容性',
    performance: '性能问题',
    security: '安全风险',
    architecture: '架构问题',
    dependency: '依赖问题',
    highRisk: '高风险',
    mediumRisk: '中风险',
    lowRisk: '低风险',
    critical: '严重',
    major: '重要',
    minor: '次要',
    negligible: '可忽略',
    fixed: '已修复',
    accepted: '已接受',
    mitigated: '已缓解',
    open: '待处理',
    riskTrend: '风险趋势',
    riskDistribution: '风险分布',
    resolutionPlan: '解决方案',
    migrationPlan: '迁移计划',
    estimatedWorkload: '预估工作量',
    priority: '优先级'
  },
  audit: {
    title: '审计日志',
    operationType: '操作类型',
    operationDesc: '操作描述',
    operator: '操作人',
    operationTime: '操作时间',
    ipAddress: 'IP地址',
    userAgent: '客户端',
    duration: '耗时',
    requestParams: '请求参数',
    responseBody: '响应结果',
    errorMessage: '错误信息',
    success: '成功',
    failed: '失败',
    viewDetail: '查看详情',
    exportLog: '导出日志',
    filterByTime: '按时间筛选',
    filterByUser: '按用户筛选',
    filterByType: '按类型筛选',
    login: '登录',
    logout: '登出',
    create: '创建',
    update: '更新',
    delete: '删除',
    query: '查询',
    upload: '上传',
    download: '下载',
    import: '导入',
    export: '导出',
    review: '审核',
    scan: '扫描',
    generate: '生成'
  },
  system: {
    title: '系统管理',
    userManagement: '用户管理',
    roleManagement: '角色管理',
    permissionManagement: '权限管理',
    dictionaryManagement: '字典管理',
    systemSettings: '系统设置',
    operationLogs: '操作日志',
    systemConfig: '系统配置',
    securitySettings: '安全设置',
    notificationSettings: '通知设置',
    themeSettings: '主题设置',
    languageSettings: '语言设置',
    userName: '用户名',
    realName: '真实姓名',
    email: '邮箱',
    phone: '手机号',
    role: '角色',
    department: '部门',
    userStatus: '用户状态',
    lastLogin: '最后登录',
    admin: '管理员',
    normalUser: '普通用户',
    viewer: '只读用户',
    dictType: '字典类型',
    dictCode: '字典编码',
    dictValue: '字典值',
    dictLabel: '字典标签',
    dictSort: '排序',
    systemInfo: '系统信息',
    systemVersion: '系统版本',
    databaseVersion: '数据库版本',
    javaVersion: 'Java版本',
    osInfo: '操作系统',
    serverTime: '服务器时间',
    systemHealth: '系统健康状态',
    memoryUsage: '内存使用',
    cpuUsage: 'CPU使用',
    diskUsage: '磁盘使用',
    activeUsers: '在线用户数',
    cleanup: '清理缓存',
    backup: '数据备份',
    restore: '数据恢复'
  },
  validation: {
    title: '验证报告',
    validationTime: '验证时间',
    validationType: '验证类型',
    validationResult: '验证结果',
    totalItems: '总项数',
    passedItems: '通过项数',
    failedItems: '失败项数',
    passRate: '通过率',
    details: '详细信息',
    codeQuality: '代码质量',
    architectureQuality: '架构质量',
    documentationQuality: '文档质量',
    testQuality: '测试质量',
    overallScore: '综合评分',
    excellent: '优秀',
    good: '良好',
    fair: '一般',
    poor: '较差',
    criticalIssue: '严重问题',
    majorIssue: '重要问题',
    minorIssue: '次要问题',
    suggestion: '改进建议',
    trendAnalysis: '趋势分析',
    comparison: '版本对比',
    generateReport: '生成报告',
    exportReport: '导出报告'
  },
  profile: {
    title: '个人中心',
    personalInfo: '个人信息',
    avatar: '头像',
    nickname: '昵称',
    gender: '性别',
    birthday: '生日',
    location: '所在地',
    bio: '个人简介',
    accountSettings: '账户设置',
    changePassword: '修改密码',
    currentPassword: '当前密码',
    newPassword: '新密码',
    confirmPassword: '确认密码',
    passwordMismatch: '两次输入密码不一致',
    securitySettings: '安全设置',
    twoFactorAuth: '两步验证',
    loginHistory: '登录历史',
    deviceManagement: '设备管理',
    notificationPreferences: '通知偏好',
    emailNotifications: '邮件通知',
    pushNotifications: '推送通知',
    language: '语言',
    timezone: '时区',
    theme: '主题'
  }
}
