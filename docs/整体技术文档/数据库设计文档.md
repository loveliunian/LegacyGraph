...
- 当前迁移版本：`V1__initial_schema.sql` 到 `V94__change_task_sandbox_and_repo_token.sql`（共 93 个脚本）
...
| `V85__graph_release_metrics.sql` | 图谱发布质量指标（metrics JSON 字段） |
| `V86__file_snapshot_archive_view.sql` | 文件快照归档视图 |
| `V87__qa_evaluation_run.sql` | QA 评测运行表 |
| `V88__seed_missing_edge_type_dict.sql` | 补齐 EdgeType 字典缺失项（59→68 项） |
| `V89__neo4j_orphan_cleanup.sql` | Neo4j 孤立节点清理 |
| `V90__process_fitness.sql` | 流程一致性校验结果表（lg_process_fitness） |
| `V91__solution_step_enhance.sql` | 方案步骤增强 |
| `V92__patch_draft.sql` | 补丁草稿表 |
| `V93__code_repo_pr_extension.sql` | 代码仓库 PR 扩展字段 |
| `V94__change_task_sandbox_and_repo_token.sql` | 变更任务沙箱与仓库 Token |
...
后端节点类型以 `NodeType.java` 为准（共 49 个），按功能域分组：
...
关系类型以 `EdgeType.java` 为准（共 59 个），按功能域分组：
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 5.0 | 2026-07-13 | 迁移版本补充至 V94（新增 V85–V94：graph_release_metrics、file_snapshot_archive_view、qa_evaluation_run、seed_missing_edge_type_dict、neo4j_orphan_cleanup、process_fitness、solution_step_enhance、patch_draft、code_repo_pr_extension、change_task_sandbox_and_repo_token）；NodeType 新增 BPMN 节点（ProcessDefinition/UserTask/ServiceTask/Gateway，共 55 个）；EdgeType 新增 BPMN 关系（FLOW_TO/RUNTIME_FLOW_TO/HAS_FLOW_NODE/EXECUTES_BY/LISTENED_BY/DEPLOYED_TO，共 68 个）；新增 lg_process_fitness 表（流程一致性校验）；EdgeType 字典项补齐至 68 项 |
| 4.0 | 2026-07-12 | 迁移版本补齐至 V84（新增 V37–V84 共 48 个脚本，V70 缺失）；Entity 修正为 73；新增需求结构化表（V61/V67/V72）、方案生成表（V65/V66/V73/V76）、图谱发布门禁（V60）、文件快照（V55/V81）、QA 测试用例与反馈（V64/V74/V78）、术语映射（V42）、扫描检查点（V83）、脚手架模板（V77）等；NodeStatus 新增 STALE（V75）；NodeType 修正为 49 个、EdgeType 修正为 59 个；Flyway `validate-on-migrate` 修正为 false、补充 `out-of-order: true` |
