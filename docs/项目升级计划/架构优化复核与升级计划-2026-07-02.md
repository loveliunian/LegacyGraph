...
## 8. 第12轮复核修正记录（2026-07-02）

> 复核输入：第6/7节执行记录 + 当前代码。  
> 代码扫描方式：CodeGraph MCP `search_graph` / `get_code_snippet` / `search_code`，文件补充读取用于测试和补丁定位。

### 发现的偏移与遗漏

| # | 类型 | CodeGraph 证据 | 修复 |
|---|---|---|---|
| 1 | **Agent 合约偏移** | `PatchPlanAgent.generate(AgentEnvelope)`、`ChangeImpactAgent.analyze(AgentEnvelope)`、`GraphMergeAgent.decideMergeFromEnvelope()`、`FeatureMappingAgent.mapFeaturesFromEnvelope()`、`TestFailureAnalysisAgent.analyzeFromEnvelope()` 仍绕回默认四参 `callWithTemplate` 或旧 API，未传递 `usedEvidenceIds/policy` | 新增 `LlmGateway.callWithEnvelope()`，统一把 `AgentEnvelope` 转为 `AgentRunContract` 并在网关内校验 `RequiredEvidencePolicy`；高风险 Agent 的 Envelope 入口改走该 Interface；`PatchPlanAgent` 对 `needsReview` 返回 `manualReviewNeeded=true` 的计划 |
| 2 | **写图对账误清** | `GraphWriteReconciler.reconcile()` 对所有 `writeStatus=INCOMPLETE` 节点/边直接 `REMOVE writeStatus, writeError`，没有确认 PG 证据关联是否补齐 | `GraphWriteReconciler` 注入 `NodeEvidenceRepository/EdgeEvidenceRepository`，仅当 `lg_node_evidence/lg_edge_evidence` 存在关联时清标记；缺证据继续保留复核队列 |
| 3 | **写图文档/Javadoc 偏移** | `GraphWriteIntent` Javadoc 指向不存在的 `GraphWriteExecutor` | 改为指向真实入口 `EvidenceGraphWriter.writeIntent()` 与 `GraphWriteReconciler` |
| 4 | **扫描双轨未真正收口** | `ProjectScanner` 仍直接 import/实例化 `JavaControllerExtractor`、`MyBatisXmlExtractor`、`VueRouteExtractor`、`FrontendApiExtractor`、`ServiceCallExtractor`、`DatabaseMetadataExtractor`；Adapter 处理数为 0 时仍回退旧扫描链 | 删除旧 Java/MyBatis/Vue/ServiceCall fallback 方法；DB 元数据抽取下沉到 `DatabaseMetadataScanService`；`ProjectScanner` 只保留 Adapter Registry 编排和 DB service 委托 |
| 5 | **架构门禁假绿** | `LayeredArchitectureTest` 三条新增规则用 `try/catch` 吞掉 `AssertionError`，只打印 advisory | task→concrete extractor、service→Neo4j Driver 改为严格 ArchUnit 规则；Agent Envelope 入口增加源码哨兵测试，防止回退默认合约 |
| 6 | **service Driver 遗留** | `Neo4jSyncService` 仍持有未使用的 `org.neo4j.driver.Driver` | 删除 Driver 字段/构造参数和死方法，测试同步改为只注入 `Neo4jGraphDao` |

### 新增/修改文件

| 类型 | 文件 |
|---|---|
| 新增生产代码 | `backend/src/main/java/io/github/legacygraph/service/DatabaseMetadataScanService.java`、`backend/src/main/java/io/github/legacygraph/service/DocumentContentService.java` |
| 修改生产代码 | `LlmGateway.java`、`PatchPlanAgent.java`、`ChangeImpactAgent.java`、`GraphMergeAgent.java`、`FeatureMappingAgent.java`、`TestFailureAnalysisAgent.java`、`GraphWriteReconciler.java`、`GraphWriteIntent.java`、`ProjectScanner.java`、`AiScanOrchestrator.java`、`Neo4jSyncService.java` |
| 新增/修改测试 | `PatchPlanAgentTest.java`、`LlmGatewayTest.java`、`GraphWriteReconcilerTest.java`、`LayeredArchitectureTest.java`、`Neo4jSyncServiceTest.java` |

### 复扫结果

| 检查项 | 结果 |
|---|---|
| `backend/src/main/java/io/github/legacygraph/task/**` concrete `extractors.*Extractor` 依赖 | ✅ 0 occurrences |
| `backend/src/main/java/io/github/legacygraph/service/**` `org.neo4j.driver.Driver` 依赖 | ✅ 0 occurrences |
| 高风险 Agent Envelope 入口回退默认 `callWithTemplate(envelope/env.getProjectId...)` | ✅ 0 occurrences |
| `GraphWriteExecutor` 错误引用 | ✅ 0 occurrences |
| `frontend/src/views/**` import `@/utils/request` | ✅ 0 occurrences |
...
| 命令 | 结果 |
|---|---|
| `mvn -q -Dtest=PatchPlanAgentTest,LlmGatewayTest,GraphWriteReconcilerTest,LayeredArchitectureTest,Neo4jSyncServiceTest test`（在 `backend/` 下执行） | ✅ 通过 |
| `mvn test`（在 `backend/` 下执行） | ✅ 523 tests / 0 failures / 0 errors / 2 skipped |
| `npm run type-check`（在 `frontend/` 下执行） | ✅ `vue-tsc --noEmit` 通过 |
