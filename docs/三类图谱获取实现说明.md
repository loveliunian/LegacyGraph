...
`FactController.extractDocFacts` 会调用 `BusinessGraphBuilder.buildBusinessGraph`，把这些结果写入 `lg_graph_node/lg_graph_edge`（**@deprecated V5 起改为 Neo4j 直写 + `EvidenceGraphWriter.upsertEdge`，本表仅用于历史兼容查询**）。
...
| `lg_fact` | 原始结构化事实 |
| `lg_graph_node` | 统一图谱节点（**@deprecated V5 起改为 Neo4j 直写**） |
| `lg_graph_edge` | 统一图谱关系（**@deprecated V5 起改为 Neo4j 直写**） |
| `lg_db_connection` | 数据库连接配置 |
| `lg_document` | 文档元信息和文件路径 |
| `lg_doc_chunk` | 文档解析后的文本切片 |
| `lg_runtime_trace` | 运行时 span |

`lg_fact` 保存的是扫描出的事实 JSON，例如 API、Mapper、前端 API 调用等。`lg_graph_node` 和 `lg_graph_edge` 保存最终统一图谱（**@deprecated V5 起改为 Neo4j 直写，新代码请写 Neo4j + `EvidenceGraphWriter.upsertEdge`**）。节点/边里会保存 `project_id`、`version_id`、`source_type`、`source_path`、`start_line`、`end_line`、`confidence`、`status`。

### 6.2 Neo4j 图数据库

> **V5 起改为 Neo4j 直写模式**：Adapter 直接写 Neo4j + 同步 PG evidence（`EvidenceGraphWriter.upsertEdge`）。
> 旧路径 `Neo4jSyncService.syncGraph(projectId, versionId)`（从 `lg_graph_node`/`lg_graph_edge` 批量同步到 Neo4j）已废弃，仅保留用于历史兼容。

当前实现（V5+）：

1. 各 Builder/Adapter 在抽取事实时直接写入 Neo4j 节点和关系，不再经过 `lg_graph_node`/`lg_graph_edge` 中转。
2. 同时通过 `EvidenceGraphWriter.upsertEdge` 将 evidence（证据：source_path、start_line、end_line 等）写入 PostgreSQL，保证图谱节点可追溯。
3. Neo4j 查询接口（`getApiCallChain`、`getTableImpact`、`getFeatureView`、`getBusinessView`）直接查 Neo4j，按 `versionId` 过滤。

历史模式（已废弃，仅供回溯参考）：`GraphBuilder.syncToNeo4j` 调用 `Neo4jSyncService.syncGraph(projectId, versionId)`，从 `lg_graph_node`/`lg_graph_edge` 查询 `status = CONFIRMED` 的节点/边按 1000 条批量写入 Neo4j。
...
`getUnifiedGraph` 直接查 PostgreSQL 的 `lg_graph_node/lg_graph_edge`（**@deprecated V5 起改为 Neo4j 直写**），按 `versionId` 和最低置信度过滤。
...
5. `lg_graph_node/lg_graph_edge`（**@deprecated V5 起改为 Neo4j 直写**）：节点和关系是否生成，`status/confidence/source_type` 是否符合预期。
...
flowchart TD
    ...
    E --> I["lg_graph_node / lg_graph_edge  @deprecated V5→Neo4j 直写"]
    F --> I
    H --> I
