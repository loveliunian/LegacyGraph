...
- 前端生产服务：Nginx `80`
- 前端开发服务：Vite `5173`
- `deploy/docker-compose.yml` 只启动前后端应用，不启动数据库、中间件和对象存储。
...
- 如果通过环境变量占位，至少配置 `DEEPSEEK_API_KEY` 或 `OPENAI_API_KEY`，并确认 `lg_llm_provider.api_config` 中的占位值已替换。
...
spring:
  datasource:
    url: ${POSTGRES_URL:jdbc:postgresql://localhost:5432/legacy_graph}
    username: ${POSTGRES_USERNAME:legacygraph}
...
审计日志写入 `sys_operation_log`，可通过前端审计页面或 SQL 查询：
...
1. 扫描是否成功完成。
2. PostgreSQL `lg_graph_node`、`lg_graph_edge` 是否有数据。
3. `Neo4jSyncService` 或 `Neo4jGraphDao` 写入是否报错。
4. 项目 ID 和版本 ID 是否一致。
...
FROM lg_graph_node
WHERE project_id = '<project_id>' AND version_id = '<version_id>' AND deleted = 0
GROUP BY node_type
ORDER BY count(*) DESC;
```
...
- `lg_graph_node(project_id, version_id)`
- `lg_graph_edge(from_node_id/to_node_id/edge_type)`
...
```sql
SELECT id, node_type, node_name, confidence, status
FROM lg_graph_node
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 1.1 | 2026-06-30 | 按当前代码更新启动、配置、Flyway、Redis、LLM、Trace、故障排查和安全操作 |
| 1.0 | 2026-06-27 | 初始版本 |
