...
- 当前迁移版本：`V1__initial_schema.sql` 到 `V30__create_semantic_cache.sql`
...
| `V6__add_privacy_and_agent_run.sql` | `lg_evidence` 新增 `privacy_level`/`redaction_policy`；新建 `lg_agent_run`（Agent 调用合约记录） |
| `V7__fix_prompt_run_column_types.sql` | `lg_prompt_run` 三列类型 JSONB → TEXT（`masked_input`/`raw_output`/`parsed_output`） |
| `V8__init_dict_seed_data.sql` | 纯数据：14 类字典 × ~70 条字典项种子数据 |
| `V9__create_change_task_tables.sql` | 新建 `lg_change_task`、`lg_patch_file`、`lg_validation_gate`、`lg_pr_task`（变更闭环） |
| `V10__dedup_evidence.sql` | 数据清理 + 新建 `content_hash` 部分唯一索引 |
| `V11__fix_llm_provider_api_config.sql` | 纯数据修复：补齐 deepseek `api_config` JSONB |
| `V12__seed_prompt_templates.sql` | 纯数据：12 个 prompt 模板种子数据 |
| `V13__add_scan_version_stats.sql` | `lg_scan_version` 新增 8 列统计缓存字段（`node_count`/`edge_count`/`fact_count`/`task_total`/`task_success`/`task_failed`/`current_stage`/`stats_updated_at`） |
| `V14__add_change_task_version.sql` | `lg_change_task` 新增 `version` 乐观锁字段 |
| `V15__create_knowledge_claim_gap_task.sql` | 新建 `lg_knowledge_claim`（知识断言）、`lg_gap_task`（知识缺口） |
| `V16__create_domain_ontology.sql` | 新建 `lg_domain_ontology_term`（术语表）、`lg_domain_ontology_relation`（术语关系） |
| `V17__switch_embedding_to_siliconflow.sql` | UPSERT `lg_llm_provider`：切换到 BAAI/bge-large-zh-v1.5（SiliconFlow） |
| `V18__fix_prompt_output_schema.sql` | UPDATE 3 条 `prompt_template` 的 `output_schema` 与 Java DTO 对齐 |
| `V19__switch_embedding_to_ollama.sql` | UPSERT `lg_llm_provider`：切换到 bge-m3（本地 Ollama，1024维） |
| `V20__create_source_asset_snapshot.sql` | 新建 `lg_source_asset_snapshot`（扫描资产快照，支持增量扫描） |
| `V21__create_graph_write_intent.sql` | 新建 `lg_graph_write_intent`（图谱写入意图 Outbox 模式） |
| `V22__create_tool_run_tables.sql` | 新建 `lg_tool_run`（工具运行记录）、`lg_tool_evidence`（工具证据） |
| `V23__rebuild_vector_document.sql` | DROP + 重建 `lg_vector_document`（修正 project_id 类型，新增 version_id/embedding/embedding_model/embedding_dim） |
| `V24__add_scan_task_progress.sql` | `lg_scan_task` 新增 `total_items`/`processed_items`/`current_item` |
| `V25__add_dict_items.sql` | 纯数据：新增 `doc_parse_status`/`change_task_type`/`change_task_status`/`graph_node_type` 字典 |
| `V26__fix_scan_task_progress.sql` | 幂等补丁：补齐 V24 可能被跳过的 3 列 |
| `V27__add_db_connection_schema_fp.sql` | `lg_db_connection` 新增 `schema_fingerprint`/`schema_fingerprint_updated_at` |
| `V28__create_ai_scan_job.sql` | 新建 `lg_ai_scan_job`（AI 扫描异步任务） |
| `V29__create_qa_tables.sql` | 新建 `lg_qa_conversation`/`lg_qa_message`/`lg_qa_feedback`（QA 对话历史） |
| `V30__create_semantic_cache.sql` | 新建 `lg_semantic_cache`（语义缓存，基于 question embedding 向量索引） |
...
`Project`、`System`、`BusinessDomain`、`BusinessProcess`、`BusinessObject`、`BusinessRule`、`Role`、`FeatureModule`、`Feature`、`Menu`、`Page`、`Button`、`Permission`、`ApiEndpoint`、`Controller`、`Service`、`Method`、`Mapper`、`SqlStatement`、`Table`、`Index`、`Column`、`ConfigItem`、`ScheduledJob`、`MQConsumer`、`MQTopic`、`ExternalSystem`、`TestCase`、`Assertion`、`Evidence`、`ChangeTask`、`Patch`、`PullRequest`、`Dependency`、`VersionRisk`。

关系类型以 `EdgeType.java` 为准，包括：

`CONTAINS`、`BELONGS_TO`、`MAPS_TO`、`IMPLEMENTED_BY`、`USES`、`EXPOSED_BY`、`HANDLED_BY`、`EXECUTES`、`READS`、`WRITES`、`HAS_COLUMN`、`HAS_INDEX`、`UNIQUE_ON`、`JOINS`、`REFERENCES`、`HAS_RULE`、`REQUIRES_PERMISSION`、`TRIGGERS`、`CONSUMES`、`CALLS_EXTERNAL`、`VERIFIED_BY`、`ASSERTS`、`HAS_EVIDENCE`、`AFFECTS`、`FIXED_BY`、`MIGRATES_TO`、`DEPENDS_ON`。
