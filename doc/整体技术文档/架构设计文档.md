...
当前文档按 2026-07-06 代码更新，核心依据：

- 后端源码：`backend/src/main/java/io/github/legacygraph/`
- 前端源码：`frontend/src/`
- 数据库迁移：`backend/src/main/resources/db/migration/`（V1–V36）
...
graph TB
    ...
    subgraph "Spring Boot 4 / Jetty"
        direction TB
        Controller["Controller 层（34 个）<br/>/lg/*, /agents, /llm/providers, /qa, /reports, /graphify"]
...
- 代码中业务接口多以 `/lg/...` 声明，少量全局接口为 `/qa`、`/agents`、`/llm/providers`、`/reports`。
...
- Actuator 暴露 `health`、`info`、`prometheus` 三个端点供监控采集。
...
├── common            # Result、PageResult、NodeType、EdgeType、NodeStatus、ScanStep、TaskStatus、ErrorCode
├── config            # Security、Redis、Flyway、Neo4j、MinIO、MyBatis、Async、health
├── controller        # HTTP API（34 个）
├── dao               # Neo4jGraphDao / Neo4jAdminRepository / Neo4jQueryRepository
├── deployment        # Graphify 部署健康指标、运维监控、生产配置
├── dto               # API、图谱、报告、Trace、Agent、claim/gap/rag/scan/trace/understanding DTO
├── entity            # MyBatis-Plus 实体（52 个）
...
├── model             # 领域模型
├── plugin            # 插件注册表（PluginRegistry/BuiltinPluginRegistrar）
...
├── security          # 出处/敏感信息脱敏
├── service           # 业务服务（change/graph/qa/report/rerank/scan/system/test 子包）
├── task              # 扫描编排和测试调度
...
> 说明：`lg_change_task`/`lg_knowledge_claim`/`lg_qa_*`/`lg_semantic_cache` 等表对应的 Service 分别落在 `service/change`、`service/graph`、`service/qa` 子包，而非顶层 `change`/`knowledge`/`qa`/`cache` 包。Graphify 相关能力分散在 `integration/graphify`、`graphify`、`deployment`、`eval`、`federation`、`governance`、`query`、`review`、`security` 等包。

### 主要 Controller

当前共 34 个 Controller（路由前缀为代码中 `@RequestMapping` 声明值，外部访问需再加 `/api` 前缀）：
...
| `QaAgent` | RAG + 图邻域问答 |
| `CodeUnderstandingAgent` | 代码理解报告生成、契约扫描 |

### 核心 Service

按 `service/` 子包分组（仅列关键服务）：
...
| | `KnowledgeClaimService` / `KnowledgeCompiler` | 知识断言、缺口发现 |
| | `EvidenceConflictService` | 证据冲突检测与处置 |
| | `GraphRagPlanExecutor` | GraphRAG 计划执行 |
| | `GraphCacheInvalidator` | 图谱相关缓存统一失效 |
...
| | `ReRankingService` + `rerank/*` | 重排序（CrossEncoder/Keyword/Document） |
| | `SemanticCache` | 语义缓存（相似问题命中） |
| | `ConversationContextManager` / `QaQualityAnalyzer` | 对话上下文与质量分析 |
| `change` | `ChangeTaskService` | 变更任务状态机 |
| | `PrOrchestrator` | PR 编排（分支/补丁/CI 触发） |
...
| | `ValidationGateRunner` / `PatchPlanValidator` | 验证门禁与补丁校验 |
| 根 | `AgentRunService` / `NotificationService` | Agent 运行记录、系统通知 |

> 扫描生命周期编排由 `task/ProjectScanner` 与 `task/AiScanOrchestrator` 承担（不在 `service` 包）；代码理解编排由 `understanding/CodeUnderstandingOrchestrator` + `CodeUnderstandingReportService` 承担。
...
- `source` 代码仓库 / 数据库 / 文档
- `scan` 创建扫描 / 任务列表 / 版本列表
- `graph` 代码 / 统一 / 业务 / 功能 / 数据血缘 / 运行时 / 图谱差异 / 图谱问答
- `fact` 事实列表 / 证据搜索
- `review` 审核列表 / 历史
- `test` 用例编辑 / 列表 / 运行详情 / 运行列表
- `report` 验证报告
- `migration` 风险列表 / 详情
- `system` 用户 / 字典 / 设置 / LLM Provider / Prompt / 插件
- `audit` 审计日志
- `agent` Agent Hub / 历史
- `change` 变更任务 / PR 工作台
- `understanding` 代码理解报告
- `vector` 向量检索
- `workbench` 证据工作台 / 质量面板 / 漂移队列 / 证据冲突 / 功能切片
