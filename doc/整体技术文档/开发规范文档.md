...
| 数据库迁移 | Flyway，手动 `FlywayConfig` 执行（V1–V36） |
...
| `controller` | HTTP Controller（34 个） |
...
flowchart TD
    ...
    SVC["service (change/graph/qa/report/scan/system/test)<br/>业务编排 / 事务边界"]
...
| Bean | 用途 | core/max/queue |
|------|------|----------------|
| `taskExecutor` | 扫描、报告、向量化等通用任务 | 4 / 8 / 100 |
| `ioTaskExecutor` | 文件上传下载、MinIO 操作 | 8 / 16 / 200 |
| `testExecutor` | API/E2E/数据库断言测试 | 2 / 4 / 50 |
...
- 当前迁移版本为 `V1` 到 `V36`（共 36 个脚本）。
- Flyway 由 `config/FlywayConfig.java` 手动创建 Bean 并在应用启动时执行 `migrate()`；配置 `out-of-order: true`、`baseline-on-migrate: true`、`clean-disabled: true`。
