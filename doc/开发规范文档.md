...
| `agent` | LLM Agent 实现 |
| `builder` | 代码/前端/业务/证据/运行时图谱构建器 |
| `common` | 通用返回、分页、枚举、错误码 |
| `config` | Spring Security、Redis、Flyway、MyBatis-Plus、Neo4j、MinIO、异步线程池配置 |
| `controller` | HTTP Controller |
| `dao` | Neo4j 访问封装 |
| `dto` | 请求、响应、报告、图谱 DTO |
| `entity` | MyBatis-Plus 表实体 |
| `exception` | 统一异常 |
| `extractors` | Java、Vue、MyBatis XML、SQL、数据库元数据、文档抽取器 |
| `filter` | Servlet 过滤器，如 JWT 认证 |
| `graph` | 图谱领域对象/辅助结构 |
| `llm` | LLM Gateway、Prompt 加载、PII 脱敏、调用异常 |
| `model` | 领域模型 |
| `parser` | 解析辅助 |
