...
> | Phase 1 | ✅ 已完成 | `AiScanOrchestrator` 接入 `ProjectScanner`：扫描后按开关执行 4 个 AI 子任务；文档事实落 `lg_fact` 并同步构建业务图谱；功能映射落待确认 `CALLS` 边和审核记录；测试用例保留请求/断言 | `AiScanOrchestratorTest`（6）、`AiScanConfigTest`（4）通过 |
> | Phase 2 | ✅ 已完成 | `QaAgent` + `GraphQaController`：向量召回+相似节点+一跳图邻域+LLM 生成，回答带证据/相关节点/置信度 | `QaAgentTest`（5 用例）、`qa-answer` 契约用例通过 |
> | Phase 3 | ✅ 已完成 | `SqlAdvisorAgent`/`TestFailureAnalysisAgent`/`ReportInsightAgent` 不只保留端点，已分别接入 MyBatis SQL 建图、测试失败回写、报告洞察接口和前端报告页 | 三个 Agent 单测 + `GraphBuilderTest`/`TestResultUpdateServiceTest`/`ReportingServiceTest` 相关用例通过 |
...
>
> 截至本轮：13 个生产 Prompt 模板全部纳入 `PromptTemplateContractTest`；本轮关联后端测试集与前端 `vue-tsc --noEmit` 已通过。注意：此前“63 个 AI 单测全部通过”的口径在本机 JDK/Mockito 环境下不可直接复现，本轮已补 `mockito-extensions/org.mockito.plugins.MockMaker` 测试配置并以实际命令验证为准。
...
- `ProjectScanner` 扫描成功后触发文档 AI 抽取、业务图谱落地、功能映射和测试生成。
...
- **`AiScanOrchestrator`**（`io.github.legacygraph.task.AiScanOrchestrator`），由 `ProjectScanner.startFullScan` 在扫描成功后调用（AI 编排失败不影响扫描 SUCCESS）：
  - `AI_DOC_EXTRACT`：读取本版本文档 → `DocUnderstandingAgent` 抽取业务域/流程/对象/规则/角色/状态流转/功能 → 写入 `lg_fact`（`sourceType=DOC_AI`，`status=PENDING_CONFIRM`），并调用 `BusinessGraphBuilder.buildBusinessGraph` 构建业务节点/边。
  - `AI_FEATURE_MAPPING`：汇总真实 `Page`/`ApiEndpoint` 节点 → `FeatureMappingAgent` 生成映射 → 创建 `sourceType=AI_FEATURE_MAPPING`、`status=PENDING_CONFIRM` 的 `CALLS` 边，并生成 `ReviewRecord` 待确认。
  - `AI_TEST_GENERATE`：`autoGenerateTestCase` 开启时，对高价值 `ApiEndpoint` 节点 → `TestCaseAgent` 生成用例并写入 `lg_test_case`（`generatedBy=LLM`，上限 20 节点），保留 preconditions、request、assertions、needHumanInput 等结构化信息。
...
- 采用“向量召回 -> 相似节点召回 -> 一跳图邻域扩展 -> 轻量去重 -> LLM 生成”。
- 回答必须返回证据列表、相关节点和置信度。
...
- **`QaAgent`**（`io.github.legacygraph.agent.QaAgent`）：
  ...
  - 图邻域：`VectorRetrievalService.findSimilarNodes` 召回语义相似节点，并通过 `Neo4jGraphDao.queryEdges/findNodesByIds` 扩展一跳上下游节点与关系。
  - 上下文拼装：节点（类型/名称/key/描述）+ 一跳关系 + 文档片段（chunk#N），同步生成结构化证据明细（`GRAPH_NODE`/`GRAPH_EDGE`/`DOC_CHUNK`）。
...
- **SQL 性能优化顾问** `SqlAdvisorAgent` + `sql-advisor.txt` + `SqlAdvisorResult`：识别 SELECT *、缺索引、N+1、前缀模糊匹配等，输出优化建议与优化后 SQL；除端点 `POST /agents/sql/analyze` 外，已接入 `GraphBuilder.buildMapperSqlGraph`，MyBatis SQL 建图时自动生成 SQL 待审核记录。
- **测试失败根因分析** `TestFailureAnalysisAgent` + `test-failure-analysis.txt` + `TestFailureAnalysis`：输入请求/响应/错误/目标节点/图谱路径/trace，输出根因、关联制品、排查步骤、是否降低置信度、重跑范围；除端点 `POST /agents/tests/analyze-failure` 外，已接入 `TestResultUpdateService.onTestFail`，失败回写时将 AI 根因摘要写入 `ReviewRecord.comment`。
- **报告洞察与行动建议** `ReportInsightAgent` + `report-insight.txt` + `ReportInsight`：输入图谱指标 + 缺口摘要，输出按优先级排序、带来源的可执行行动清单；除端点 `POST /agents/report/insights` 外，新增 `GET /lg/projects/{projectId}/reports/insights`，`ValidationReport.vue` 已替换硬编码建议并消费真实 action 列表。
...
按"先修契约，再接主流程，最后扩能力"路线，Phase 0–4 已基本落地。本轮复核后补齐了此前只停在 Agent/端点层的主流程接入：
...
- **Phase 1**：`AiScanOrchestrator` 接入 `ProjectScanner`，扫描后按 `enableAi/autoGenerateTestCase/minConfidence` 执行 4 个 AI 子任务（`AI_DOC_EXTRACT`/`AI_FEATURE_MAPPING`/`AI_TEST_GENERATE`/`AI_REVIEW_PREPARE`）；已修复 `PAGE/API` 与真实 `Page/ApiEndpoint` 标签不一致问题，文档事实同步建业务图谱，功能映射同步落待确认边和审核记录。
- **Phase 2**：`QaAgent` + `GraphQaController` 自然语言问答（向量召回+相似节点+一跳图邻域+LLM 生成，带证据/相关节点/置信度）。
- **Phase 3**：SQL 性能顾问、测试失败根因分析、报告行动建议三个新 Agent + 端点，并已接入 MyBatis SQL 建图、测试失败回写、报告洞察接口与前端报告页。
...
- 后端：`mvn -q -Dtest=AiScanOrchestratorTest,QaAgentTest,GraphBuilderTest,TestResultUpdateServiceTest,ReportingServiceTest,LlmGatewayTest,PromptTemplateContractTest,SqlAdvisorAgentTest,TestFailureAnalysisAgentTest,ReportInsightAgentTest,Phase4AgentsTest test`
- 前端：`npm run type-check`

**仍后置**：微服务拆分建议仍未实现；QA 的“规则过滤/专用 reranker”仍是轻量实现（当前为向量召回、相似节点、一跳邻域和去重），后续可继续增强。
