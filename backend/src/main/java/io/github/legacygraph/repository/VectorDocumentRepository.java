...
public interface VectorDocumentRepository extends LegacyBaseMapper<VectorDocument> {
     ...
     */
    default List<VectorDocument> findSimilarByEmbedding(
            Long projectId, List<Double> embedding, int topK, String chunkType) {
        ...
        if (chunkType != null && !chunkType.isBlank()) {
            return findSimilarByEmbeddingWithType(projectId, embeddingStr, topK, chunkType);
        } else {
            return findSimilarByEmbeddingWithoutType(projectId, embeddingStr, topK);
        }
    }
}
