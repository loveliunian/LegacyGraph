...
public interface EvidenceRepository extends LegacyBaseMapper<Evidence> {
     ...
     * <code>CREATE UNIQUE INDEX ON lg_evidence(content_hash)
     * WHERE content_hash IS NOT NULL AND deleted = 0</code>
     * <p>
     * ON CONFLICT WHERE 子句必须包含索引的全部谓词，否则 PostgreSQL 会报：
     * "there is no unique or exclusion constraint matching the ON CONFLICT specification"
     * </p>
     *
     * @param evidence 证据实体（contentHash 非空且 deleted=0 时才走冲突检测）
