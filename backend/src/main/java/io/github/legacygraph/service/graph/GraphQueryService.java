...
public class GraphQueryService {
    ...
    private List<Map<String, Object>> getTableImpactUncached(String projectId, String versionId, String tableName) {
        ...
        // 构建边列表（去重，前端兼容 source/target + startNodeId/endNodeId 双格式）
        Set<String> edgeKeys = new LinkedHashSet<>();
        List<Map<String, Object>> edgesList = new ArrayList<>();
        if (chain.edges != null) {
            for (var edge : chain.edges) {
                ...
                if (edgeKeys.add(key)) {
                    ...
                    e.put("id", edge.id());
                    e.put("type", edge.type());
                    e.put("label", edge.type());                 // 前端同时检查 type 和 label
                    e.put("source", edge.source());
                    e.put("target", edge.target());
                    e.put("startNodeId", edge.source());         // 兼容旧前端字段
                    e.put("endNodeId", edge.target());
                    edgesList.add(e);
                }
            }
        }

        // 包装为单元素数组，每项含 nodes + edges（兼容前端迭代 path 的格式）
    ...
    private List<Map<String, Object>> getTablesNodesUncached(String projectId, String versionId) {
        ...
        // 批量查询每个表的 columnCount (HAS_COLUMN 出边) 和 relationCount (REFERENCES 入/出边)
        Map<String, long[]> statsMap = queryTableStats(projectId, normalizedVersionId,
                tableNodes.stream().map(GraphNode::getId).toList());
        ...
        return tableNodes.stream().map(node -> {
            ...
            long[] stats = statsMap.getOrDefault(node.getId(), new long[]{0, 0});
            m.put("columnCount", stats[0]);
            m.put("relationCount", stats[1]);
            return m;
        }).toList();
    }

    /**
     * 批量查询 Table 节点的 columnCount 和 relationCount。
     * 一条 Cypher 返回每个 nodeId 对应的 HAS_COLUMN 出边数和 REFERENCES 出/入边数。
     */
    private Map<String, long[]> queryTableStats(String projectId, String versionId, List<String> nodeIds) {
        return neo4jGraphDao.queryTableStats(projectId, versionId, nodeIds);
    }
